...
def teams_list(request):
    ...
    if request.method == 'GET':
        ...
        context['create_by_users'] = users.filter(is_active=True, id__in=user_ids)
        return render(request, 'teams.html', context)

    if request.method == 'POST':
        if request.POST.get('team_name'):
            queryset = queryset.filter(name__incotains=request.POST.get('team_name'))
        ...
        if request.POST.get('assigned_to'):
            queryset = queryset.filter(users__id__in=request.POST.getlist('assigned_to'))
